...
valid = valid.sort(['user_id','impression_id'])
valid = valid.collect()
valid = valid.to_pandas()
# sample_ids = valid['user_id'].unique()[:5000]
...
model = CatBoostRanker(iterations=5000,
                              random_state=2024,
                              max_depth=8,
                              #max_leaves=32,
                              learning_rate=0.05,
...
model.save_model(f'./models/catboost_query_loss_0620_all.cat')
...
# model.load_model(f'./models/catboost_pair_rank_loss_0619_all.cat')
...
gauc = 0.866 ##offline score 

print(len(feature_name))

infer = test[["impression_id", "article_ids_inview", "user_id"]]
infer['pred'] = online_preds
infer.to_parquet(f"./oof/test_oof_0620_query_loss_{round(gauc, 4)}_all.parquet")
...
shutil.make_archive(f"../submit/0620_query_loss_gauc={round(gauc, 4)}_all", "zip", "../submit", "predictions.txt")
