...
not_use_columns = [
    ...
    "article_freq",
    "article_id_fixed",
    "impr_article_impr_hour_article_cnt_rank_reverse_right",
    "total_inviews_right",
    "impr_article_total_inviews_rank_right"
...
model = CatBoostRanker(iterations=10000,
                              random_state=2024,
                              max_depth=8,
                              #max_leaves=32,
                              learning_rate=0.05,
                       ...
                       train_dir='cat_query',
                        # bootstrap_type='Bernoulli',
                            ...
                            # custom_metric=['AUC', 'NDCG:top=10'],
                              #thread_count=16,
                              #scale_pos_weight=10
                              loss_function="QueryCrossEntropy"#"QueryCrossEntropy"#'PairLogitPairwise'#"QueryCrossEntropy",#'PairLogitPairwise'
                           )
...
model.save_model(f'./models/catboost_query_loss_0620.cat')
...
# {'learn': {'PairLogit': 0.2541925648114102},
 # 'validation': {'PairLogit': 0.34311628441140013}}
# {'learn': {'PairLogit': 0.24430136900981494},
 # 'validation': {'PairLogit': 0.3265025453653809}}
    
# {'learn': {'PairLogit': 0.2096073651490208},
#  'validation': {'PairLogit': 0.2839623310399261}}

# bestTest = 0.283962331
# bestIteration = 3567
...
preds = model.predict(valid[feature_name])#model.predict(valid[feature_name])#model.predict_proba(valid[feature_name])#model.predict(valid[feature_name])#model.predict_proba(valid[feature_name])
...
gauc = np.mean(aucs)
print(global_auc,gauc) #(0.8989176462952766, 0.8659283719350928)

val_sub = valid[['impression_id','article_ids_inview','user_id']]
val_sub['pred'] = preds
val_sub.to_parquet(f"./oof/val_0620_query_oof_{round(gauc, 4)}.parquet")
# val_sub.to_parquet(f"./oof/val_0605_query_cross_entropy_oof_{round(gauc, 4)}.parquet")

# imp.to_csv(f"./oof/imp_catboost_{round(gauc, 4)}.csv")
# imp.to_csv(f"./oof/imp_catboost_0601_query_cross_entropy_{round(gauc, 4)}.csv")
imp.to_csv(f"./oof/imp_catboost_0620_query_loss_{round(gauc, 4)}.csv")
...
# model.load_model(f'./models/catboost_pair_rank_loss_0609.cat')
...
infer.to_parquet(f"./oof/test_oof_0620_query_loss_{round(gauc, 4)}_train.parquet")
...
shutil.make_archive(f"../submit/0620_query_loss_gauc={round(gauc, 4)}_train", "zip", "../submit", "predictions.txt")
