...
model.save_model(f'./models/catboost_pair_rank_loss_0620_all.cat')
...
gauc = 0.8601 ###offline score 

print(len(feature_name))

infer = test[["impression_id", "article_ids_inview", "user_id"]]
infer['pred'] = online_preds
infer.to_parquet(f"./oof/test_oof_0620_pair_rank_loss_{round(gauc, 4)}_all.parquet")
...
shutil.make_archive(f"../submit/0620_pair_rank_loss_gauc={round(gauc, 4)}_all", "zip", "../submit", "predictions.txt")