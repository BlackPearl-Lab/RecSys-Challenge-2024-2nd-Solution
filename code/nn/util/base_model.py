...
class CusBaseModel(nn.Module):
    def __init__(self, 
                 feature_map, 
                 model_id="CusBaseModel", 
                 ...
                 gpus=None,
                 **kwargs):
        super(CusBaseModel, self).__init__()
        self.device = get_device(gpu) if gpu >= -1 else torch.device('cuda')
        self.gpus=gpus
        print('device', self.device, 'gpus',gpus)
        ...
        if model_id_suffix is not None:
            self.model_dir = os.path.join(kwargs["model_root"], feature_map.dataset_id + '_' + model_id_suffix)
        else:
            self.model_dir = os.path.join(kwargs["model_root"], feature_map.dataset_id)
        self.checkpoint = os.path.abspath(os.path.join(self.model_dir, self.model_id + ".model"))
        
        print(f"ckpt_path={self.checkpoint}")
    ...
    def fit(self, data_generator, epochs=1, validation_data=None,
        ...
        logging.info(f'eval steps={self._eval_steps}')
        logging.info("Start training: {} batches/epoch".format(self._steps_per_epoch))
        logging.info("************ Epoch=1 start ************")
        for epoch in range(epochs):
            self._epoch_index = epoch
            self.train_epoch(model, data_generator)
    ...
    def train_epoch(self, model, data_generator):
        self._batch_index = 0
        train_loss = 0
        self.train()
        if self._verbose == 0:
        ...
        for batch_index, batch_data in enumerate(batch_iterator):
            ...
            self._total_steps += 1
            loss = self.train_step(model, batch_data)
    ...
    def evaluate(self, data_generator, metrics=None):
        ...
        with torch.no_grad():
            ...
            logging.info('finish infer on valid data, begin evaluate......')
            
            group_id = np.array(group_id) if len(group_id) > 0 else None
            if metrics is not None:
                self.val_logs = self.evaluate_metrics(y_true, y_pred, metrics, group_id)
            else:
                self.val_logs = self.evaluate_metrics(y_true, y_pred, self.validation_metrics, group_id)
            logging.info('[Metrics] ' + ' - '.join('{}: {:.6f}'.format(k, v) for k, v in self.val_logs.items()))
            return self.val_logs
